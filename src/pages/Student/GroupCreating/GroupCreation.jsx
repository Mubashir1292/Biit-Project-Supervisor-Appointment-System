...
function GroupCreation() {
  ...
  const handleGetOptions = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/GettingTechnolgiesOtherThenCreatorTechnology?regNo=${user.uid}`
      );
      const data = await response.json();
      if (data) {
        setOptions(data);
  ...
  const checkingGroupExistance = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/ChekingGroupExistence?id=${user.uid}`
      );
      const data = await response.json();
      if (data === 0) {
  ...
  const GetGroupCgpa = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/GroupCgpaByRegNo?regNo=${user.uid}`
      );
      const data = await response.json();
  ...
  const handleSubmit = async () => {
    const response = await fetch(
      `http://192.168.137.18/OfficialPSAS/api/psas/CreateNewGroup?regNo=${user.uid}&title=${title}&desc=${desc}&creatorTechnology=${selection.value}`,
      {
        method: "POST",
        headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/CheckingStudentGroupStatus?regNo=${completeAridNumber}`
      );
      const data = await response.json();
      console.log(data);
      if (data.includes("0")) {
  ...
  const handleRequest = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/SendingRequestToMember?senderId=${user.uid}&receiverId=${completeAridNumber}&technology=${selection.label}&Message=${message}`,
        {
          method: "POST",
          headers: {
