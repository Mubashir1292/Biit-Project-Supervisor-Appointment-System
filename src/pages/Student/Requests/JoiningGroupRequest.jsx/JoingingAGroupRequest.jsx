...
function JoingingAGroupRequest() {
  ...
  const getGroupsOnTechnologyBase = async () => {
    try {
      const response = await fetch(
        `http://192.168.43.100/OfficialPSAS/api/psas/GroupsFetching?techName=${selectedDomain.value}&regNo=${user.uid}`
      );
      const data = await response.json();
      if (data !== "not Founded") {
  ...
  const getDetailsAboutProjectAndGroup = async () => {
    try {
      const response = await fetch(
        `http://192.168.43.100/OfficialPSAS/api/psas/GetTheDetails?group_id=${choosedProject.gid}&project_id=${choosedProject.pid}`
      );
      const data = await response.json();
      console.log(data);
      if (data !== null) {
  ...
  const handlePostingRequesting = async () => {
    const response = await fetch(
      `http://192.168.43.100/OfficialPSAS/api/psas/PostingRequestForGroupJoining?regNo=${user.uid}&gid=${choosedProject.gid}&tecid=${selectedDomain.label}&message=${message}`,
      {
        method: "POST",
        headers: {
