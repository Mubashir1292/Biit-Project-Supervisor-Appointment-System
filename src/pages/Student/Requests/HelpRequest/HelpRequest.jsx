...
function HelpRequest() {
  ...
  const gettingTechnology = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/GetTechnology?regNo=${user.uid}`
      );
  ...
  const gettingExpertsUsingTechnology = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/FetchingRelativeTechnicalExpert?regNo=${user.uid}`
      );
      const data = await response.json();
      console.log(data);
      if (data !== null) {
  ...
  const getAllWorkingDays = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/GetAllDays`
      );
      const data = await response.json();
      if (data !== null) {
  ...
  const getTimeSlots = async () => {
    try {
      const response = await fetch(
        `http://192.168.137.18/OfficialPSAS/api/psas/GetTheTimeSlots?day=${
  ...
  const postingHelpRequest = async () => {
    try {
      if (selectionExpert.label && user.uid && Timeselection.label && message) {
        const response = await fetch(
          `http://192.168.137.18/OfficialPSAS/api/psas/PostingHelpRequest?teacher_id=${
